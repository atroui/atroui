/**
 * Compact registry family map — short slugs, one @atroui/ legend.
 * Gradual revelation: browse the gallery for the full list; this is the shape.
 */
const FAMILIES: { name: string; items: string[] }[] = [
  {
    name: "Libs",
    items: ["utils", "motion", "brand", "site-url", "seo", "analytics"],
  },
  {
    name: "UI",
    items: [
      "button",
      "card",
      "textarea",
      "input",
      "field",
      "checkbox",
      "radio",
      "switch",
      "accordion",
      "tabs",
      "dialog",
      "alert-dialog",
      "menu",
      "popover",
      "tooltip",
      "toast",
      "progress",
      "slider",
      "meter",
      "avatar",
      "form-select",
      "breadcrumbs",
      "prose",
      "autocomplete",
      "checkbox-group",
      "collapsible",
      "combobox",
      "context-menu",
      "drawer",
      "fieldset",
      "form",
      "menubar",
      "navigation-menu",
      "number-field",
      "otp-field",
      "preview-card",
      "scroll-area",
      "separator",
      "toggle",
      "toggle-group",
      "toolbar",
    ],
  },
  {
    name: "Motion",
    items: [
      "fade-in",
      "stagger",
      "line-reveal",
      "word-reveal-scroll",
      "text-morph",
      "copy-button",
      "label-roll",
      "magnetic",
      "tilt",
      "spotlight",
      "marquee",
      "border-beam",
      "magic-card",
      "animated-beam",
      "bento-grid",
      "meteors",
      "dock",
      "orbiting-circles",
      "number-ticker",
      "aurora-text",
      "animate-number",
      "transition-panel",
      "scroll-progress",
      "timeline-animation",
    ],
  },
  {
    name: "Theme",
    items: ["theme-toggle", "theme-adapt", "theme-provider"],
  },
  {
    name: "Chrome",
    items: ["site-header", "site-footer", "footer-bold", "logo", "founder-avatar"],
  },
  {
    name: "Home",
    items: ["home-hero", "home-who", "home-principle", "home-work", "home-lab", "hero-parallax", "hero-lamp", "logo-marquee", "saas-starter"],
  },
  {
    name: "Convert",
    items: [
      "pricing-overview",
      "pricing-tiers",
      "faq",
      "faq-accordion",
      "contextual-cta",
      "cta-band",
      "exit-intent-popup",
      "feature-bento",
      "testimonials-stack",
      "stats-band",
      "integration-beam",
      "showcase-carousel",
      "auth-split",
    ],
  },
  {
    name: "Capture",
    items: [
      "waitlist-form",
      "newsletter-form",
      "calendly-embed",
      "made-with-embed",
    ],
  },
  {
    name: "Content",
    items: ["social-share", "journal-content", "resources-content"],
  },
  {
    name: "Studio",
    items: ["ar-portfolio", "live-dashboard", "project-planner"],
  },
  {
    name: "SEO",
    items: ["analytics-provider"],
  },
  {
    name: "Tools",
    items: [
      "og-live-preview",
      "og-examples",
      "og-workspace",
      "thumbnail-live-preview",
      "thumbnail-workspace",
      "scope-chat",
    ],
  },
  {
    name: "Host APIs",
    items: [
      "api-contact",
      "api-waitlist",
      "api-newsletter",
      "api-generate",
      "api-thumbnail",
      "api-scope",
    ],
  },
]

export function DocsRegistryMap() {
  return (
    <dl className="docs-registry-map">
      {FAMILIES.map((family) => (
        <div key={family.name} className="docs-registry-map-row">
          <dt>{family.name}</dt>
          <dd>
            {family.items.map((item) => (
              <code key={item}>{item}</code>
            ))}
          </dd>
        </div>
      ))}
    </dl>
  )
}
