...
function ChangelogBody({ body }: { body: string }) {
  ...
  const flushList = () => {
    ...
    nodes.push(
      <ul
        key={`ul-${nodes.length}`}
        className="list-disc space-y-2 pl-5 text-[15px] leading-relaxed"
      >
        {list.map((item, i) => (
  ...
  for (const line of lines) {
    ...
    if (heading) {
      ...
      nodes.push(
        <h3
          key={`h-${nodes.length}`}
          className="docs-section-title !text-[0.9375rem] !tracking-normal"
        >
          {heading[1]}
        </h3>,
      )
  ...
  return (
    <article className="space-y-10">
      <DocsArticleHeader
        ...
        lede={
          <>
            Released versions of <code>atroui</code>. Entries are written via
            Changesets on each PR, then published when the Version Packages PR
            merges.
          </>
        }
      />
      ...
      <div className="space-y-10">
        {releases.map((release) => (
          <section key={release.version} className="space-y-4">
            <h2 className="docs-section-title" id={release.version}>
              {release.version}
            </h2>
