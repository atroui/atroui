...
import { getPseoCollection, getPseoPage } from "@/lib/pseo"
...
export function DocsCollectionPageView({ slug }: { slug: string }) {
      ...
      <section className="space-y-3">
        ...
        <ProductPanel>
          {collection.paths.map((href) => (
            <ProductPanelLink key={href} href={href} title={labelFor(href)} />
          ))}
        </ProductPanel>
      </section>
